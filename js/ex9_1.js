...
const imClube = document.querySelector("#imgClube") // Seleção da imagem no html 
...
const trocarClube = () => {
    ...
    imClube.className = "img-fluid img-clube" // Muda o estilo para exibir a imagem
    imClube.alt = `Símbolo do ${clube}` // Modifica o atributo alt 

    localStorage.setItem("clube", clube) // Salva a variável clube no navegador a escolha do cliente
}
...
frm.rbBarcelona.addEventListener("change", trocarClube)
frm.rbSantos.addEventListener("change", trocarClube)
